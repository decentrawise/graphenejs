...
class ChainStore {
    ...
    requestAllDataForAccount(account_id, object_type) {
        ...
        if (
            current.toJS().more_data_available.balances &&
            object_type == "balance"
        ) {
            Apis.instance()
                .db_api()
        ...
        if (
            ...
            Apis.instance()
                ...
                .then((results) => {
                    ...
                    account.vesting_balances =
                        account.vesting_balances.withMutations((set) => {
                    ...
                    account.htlcs_to = account.htlcs_to.withMutations((set) => {
        ...
        pending_request.promise = new Promise((resolve, reject) => {
            Apis.instance()
                ...
                .then((operations) => {
                    ...
                    let pending_request =
                        this.account_history_requests.get(account_id);
                ...
                if (result.length > 0) {
                    ...
                    result.forEach((pool) => {
                        ...
                        pool.dynamic_share_asset = this.getObject(
                            pool.share_asset.get("dynamic_asset_data_id")
                        );
                        tmp.push(pool);
                    });
    ...
    getLiquidityPoolsByShareAsset(assets, subscribe = false) {
        return Apis.instance()
            ...
            .catch((error) => {
                console.error(
                    "get_liquidity_pools_by_share_asset error:",
                    error
                );
            });
    }
