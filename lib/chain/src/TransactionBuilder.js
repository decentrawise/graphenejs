    ...
    process_transaction(cwallet, signer_pubkeys = null, broadcast = false) {
        ...
        return this.set_required_fees().then(() => {
            ...
            if (signer_pubkeys) {
                ...
                // potential keys.
                var pubkeys =
                    cwallet.getPubkeys_having_PrivateKey(signer_pubkeys);
                        ...
                        (required_pubkeys) => {
                            for (let pubkey_string of required_pubkeys) {
                                if (signer_pubkeys_added[pubkey_string])
                                    continue;
                                var private_key =
                                    cwallet.getPrivateKey(pubkey_string);
                                if (!private_key)
    ...
    */
    ...
    set_required_fees(asset_id, removeDuplicates) {
        ...
        if (feeAssets.length > 1 || feeAssets[0] !== "1.3.0") {
             ...
             */
            let dynamicObjectIds = feeAssets.map((a) =>
                a.replace(/^1\./, "2.")
            );
            promises.push(
                Apis.instance()
            ...
            promises.push(
                Apis.instance().db_api().exec("get_objects", [dynamicObjectIds])
            );
        }
